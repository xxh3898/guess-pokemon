...
- GitHub Actions는 `dev` push와 `main` 대상 PR에서 frontend, backend, infra, API image, Web image 검증을 독립 job으로 실행한다. backend test container는 runner의 Gradle wrapper·dependency cache를 재사용하지만 test task 결과 cache는 사용하지 않는다. ARM64 image 검증과 publish는 native `ubuntu-24.04-arm` runner에서 실행해 QEMU emulation을 거치지 않는다. 보호된 `main`은 PR 필수 검증을 통과한 commit만 받으므로 push 뒤에는 image 발행과 배포만 실행한다.

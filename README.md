...
한 명은 전국도감에서 정답 포켓몬을 고르고, 다른 한 명은 자유롭게 질문하거나 포켓몬을 추측합니다. 매 경기 전에 원하는 역할을 고를 수 있습니다.

## 어떤 게임인가요?

1. 두 사용자가 회원가입하고 같은 방에 입장합니다.
2. 두 사용자가 원하는 역할을 고릅니다. 선택이 겹치면 서버가 무작위로 역할을 정합니다.
3. 질문자는 포켓몬의 특징을 질문하거나 전국도감에서 정답을 추측합니다.
4. 출제자는 `예`, `아니요`, `모르겠어요`로 답하고 필요한 경우 코멘트를 덧붙입니다.
5. 질문과 추측을 합쳐 20번 안에 정답을 맞히면 질문자가 승리합니다.
6. 경기가 끝나면 역할을 다시 골라 다음 경기를 시작합니다.
...
| 역할 선택·재대결 | 서로 다른 선호는 그대로, 같은 선호는 무작위로 배정해 다음 경기 진행 |
